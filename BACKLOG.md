...
| **The startup banner printed the password too** — FIXED | The success path printed `dbConfig.uri` verbatim twice ("Database :" and "open Compass at"), so a healthy deploy wrote the Atlas password into Render's retained logs. Redacted; a local `mongodb://` URI has no credentials so it still prints in full and stays copy-pasteable into Compass. |
| **Banner advertised localhost on a deployed host** — FIXED | Health and signalling URLs now use `RENDER_EXTERNAL_URL` when present. |
| **`GET /` returned 404** — FIXED | Opening the service URL gave a bare 404 and filled the logs with `Route not found: /`, which reads as a broken deployment rather than an API with no homepage. Now returns a small JSON pointer to `/api/health`; `/favicon.ico` returns 204. |
