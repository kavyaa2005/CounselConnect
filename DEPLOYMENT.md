...
Your code is already on GitHub — that repository is what both hosts deploy from.
Make sure you connect the host to the same repo and branch you actually push to;
if your team has more than one fork, deploying the wrong one is a confusing
half-hour.
...
> can see `render.yaml` and `vercel.json` in the file list, and that
> `pnpm-lock.yaml` is **gone**. If `render.yaml` is missing the push didn't go
> through; if `pnpm-lock.yaml` is still there, both hosts will try to build with
> pnpm and fail.
...
> **This step is entirely inside MongoDB Atlas. You do not open Render, and you
> do not buy anything.** It takes about 20 seconds.

Atlas ships locked down: it refuses every incoming connection until you tell it
which addresses are permitted. Right now that list is empty, so even a correct
password would be rejected. You need to add one entry.

Still in Atlas (**not** Render):

1. In the left sidebar, click **Network Access**.
2. Click the green **Add IP Address** button.
3. Click **Allow Access from Anywhere**. The address box fills in with
   `0.0.0.0/0` — that is the "any address" wildcard.
4. Click **Confirm**.

Wait until the status next to the entry turns from *Pending* to **Active**
(about a minute). That's the whole step.

**Why "anywhere" and not one specific address?** Because you don't have one
address to name. Render's free plan runs your service from a shared pool and
gives it a different outbound address whenever it restarts, so there is nothing
stable to allowlist. Pinning one down needs a paid static-IP add-on — that is
the package Render tries to sell you, and **you do not need it.** Everyone
deploying a hobby project on a free host uses `0.0.0.0/0`.

**Is that safe?** It means anyone on the internet can *reach* your database's
front door — but not open it. They would still need the username and password
from step 2.2, which exist only in Render's environment variables. So the
password is what protects your data, not the address list.

What it does mean is that your database password is now the only thing standing
between the internet and your data, so don't commit it, don't paste it into
chat, and use the autogenerated one rather than something you invented.

(For genuinely sensitive production data — real patients, not a demo — you'd pay
for that static IP add-on so the address list becomes a second lock. That's a
real-world footnote, not something to do for this project.)
...
### 2.5 Test the string before you deploy with it

Atlas reports nearly every credential problem with the same three words — *bad
auth : authentication failed* — whether the password placeholder was never
replaced, the angle brackets were left in, the password contains an unescaped
`@`, or the user lives in a different Atlas project. Finding that out from
Render's logs is slow, so check it on your laptop first:

```bash
cd backend
npm run db:check -- "PASTE_YOUR_CONNECTION_STRING_HERE"
```

It inspects the string, names any problem it finds with the exact fix, then
actually connects and confirms the user can write. It never prints your
password.

A healthy result looks like:

```
The string looks well-formed. Trying to connect…

Connected successfully.
  database    counselconnect
  collections 0
  (empty — the server will import backend/data/*.json on first boot)
  write access confirmed
```

Only once you see that should you paste the string into Render. Run it any time
with no argument to check whatever is in your `backend/.env`:
...
There are two ways to create a Render service, and the difference matters:

- **Blueprint** — Render reads `render.yaml` from your repo and configures
  everything itself.
- **Web Service** — Render *ignores* `render.yaml` and guesses the settings
  from your files. You then fix them by hand.

**Use Blueprint.** It is fewer steps and it cannot guess wrong.

#### Option A — Blueprint (recommended)

1. Log in to Render → **New +** → **Blueprint**.
2. Pick your `CounselConnect-` repository and click **Connect**.
3. Render finds `render.yaml` and shows the service it's about to create. It
   will prompt you for the secret values — that's `MONGODB_URI`,
   `FRONTEND_URL`, and the two Razorpay keys. Fill in `MONGODB_URI` now and
   leave the rest blank for the moment.
4. **Apply**.

Skip to 3.3.

#### Option B — Web Service (if you already started this way)

Render will have auto-detected the wrong build command, because your repository
contains a React app at the top level and the API inside `backend/`. Go to
**Settings** and set each of these by hand:
...
Three of those are worth explaining, because each produces a confusing error if
it's wrong:

- **Root Directory must be `backend`.** Left blank, Render installs the
  frontend's packages and then can't find `server.js`.
- **Build Command must be `npm install`** — not the `pnpm install
  --frozen-lockfile; pnpm run build` that Render suggests. The backend is plain
  Node; there is nothing to build. If you leave the suggestion in place you get
  `ERR_PNPM_NO_SCRIPT Missing script: build`, which is correct — there isn't one,
  and there shouldn't be.
- **Start Command must be `npm start`.**

After changing these, click **Manual Deploy** → **Deploy latest commit**.
Settings changes alone don't rebuild.
...
| Render logs: `MongoDB is required but unreachable: bad auth : authentication failed` | Atlas rejected the **username or password** — this is not the IP list | Run `npm run db:check -- "your string"` locally; it names the exact cause. Most often the `<db_password>` placeholder was never replaced, or the password contains an unescaped `@` |
| Render logs: `MongoDB is required but unreachable` (timeout, not bad auth) | Atlas is refusing the **connection** | Network Access → allow `0.0.0.0/0`, and wait for status **Active** |
...
| Build fails: `ERR_PNPM_NO_SCRIPT Missing script: build` | Render is running its guessed pnpm command instead of yours. The backend is plain Node and has no build step | Set Build Command to `npm install` and Start Command to `npm start` (see 3.1 Option B), then Manual Deploy |
| Build fails: `ERR_PNPM_LOCKFILE_CONFIG_MISMATCH ... "overrides" configuration doesn't match` | A stale `pnpm-lock.yaml` in the repo root, out of sync with `package.json`. It also makes both hosts pick pnpm over npm | Already fixed — the stale pnpm lockfile and workspace file were deleted and gitignored. If it reappears, delete `pnpm-lock.yaml` and `pnpm-workspace.yaml` from the repo root and push |
| Render log: `Using Node.js version 26.x` and odd crashes | An open-ended engines range let Render pick the newest Node | `backend/package.json` now pins `"engines": {"node": "22.x"}` — the version everything was tested on |
