...
import {
  ...
  LogOut, Heart, PanelLeftClose, PanelLeftOpen, ClipboardCheck
} from "lucide-react";
...
const navGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "applications" as Page, label: "Applications", icon: ClipboardCheck },
...
export function Sidebar({ collapsed, onToggle, activePage, onNavigate }: SidebarProps) {
  ...
  // Live count of counselor applications waiting on review
  const [pendingApps, setPendingApps] = useState(0);
  useEffect(() => {
    let stop = false;
    const load = () => api.get("/admin/applications?status=pending")
      .then(r => { if (!stop) setPendingApps(r.data?.counts?.pending || 0); })
      .catch(() => {});
    load();
  ...
  // Invalidate the token server-side, drop local state, then hard-reload to /login
            ...
            <div className={cn("space-y-0.5", collapsed && "flex flex-col items-center")}>
              {group.items.map(item => (
                <NavItem key={item.id} {...item}
                  badge={item.id === "applications" ? (pendingApps || undefined) : (item as any).badge}
