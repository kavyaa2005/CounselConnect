...
const tabs = [
  ...
  { id: "api", label: "Access & Log", icon: Key },
...
export function Settings(_props?: any) {
  const { t } = useTheme();
  const { data, loading, error, refetch } = useAdminData(adminApi.settings);
  ...
  const [security, setSecurity] = useState({
    twoFA: false, sessionTimeout: true, ipWhitelist: false, auditLog: true, strongPassword: true,
  });
  ...
  // Seed local form state from whatever the server has stored
  useEffect(() => {
    if (!data?.settings) return;
    const s = data.settings;
    if (s.organization) {
      setOrgName(s.organization.name ?? "CounselConnect");
      setOrgEmail(s.organization.email ?? "");
      setOrgPhone(s.organization.phone ?? "");
      setTimezone(s.organization.timezone ?? "Asia/Kolkata");
    }
    if (s.adminNotifications) setNotifs(n => ({ ...n, ...s.adminNotifications }));
    if (s.security) {
      setSecurity({
        twoFA: !!s.security.twoFactor,
        sessionTimeout: (s.security.sessionTimeoutMinutes || 0) > 0,
        ipWhitelist: !!s.security.ipWhitelist,
        auditLog: !!s.security.auditLogging,
        strongPassword: !!s.security.strongPassword,
      });
    }
    if (s.hours) setHours(h => ({ ...h, ...s.hours }));
  ...
  const apiKeys = [
    {
      ...
      key: "configured in backend/.env — never exposed to the browser",
      created: "Set at install",
      lastUsed: "Every request",
    },
  ];
  ...
  async function handleSave() {
    ...
    try {
      await adminApi.updateSettings({
        organization: { name: orgName, email: orgEmail, phone: orgPhone, timezone },
        general: { platformName: orgName, supportEmail: orgEmail, timezone },
        adminNotifications: notifs,
        security: {
          twoFactor: security.twoFA,
          sessionTimeoutMinutes: security.sessionTimeout ? 60 : 0,
          ipWhitelist: security.ipWhitelist,
          auditLogging: security.auditLog,
        ...
        <div className="flex-1 min-w-0">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab === "api" && (
                <div className="space-y-5">
                  <h3 className="font-semibold text-base" style={{ color: t.text, fontFamily: "'Poppins', sans-serif" }}>Access & Secrets</h3>
                  <div className="p-4 rounded-xl flex items-start gap-2.5" style={{ background: "#FFFBEB", border: "1px solid #F59E0B25" }}>
                    ...
                    <p className="text-xs" style={{ color: "#92680B" }}>
                      The platform signing secret lives in <code>backend/.env</code> as <code>JWT_SECRET</code> and is
                      never sent to the browser. Change it there and restart the API to rotate all sessions.
                    </p>
                  </div>
                    ...
                    <div key={k.id} className="p-4 rounded-xl border" style={{ borderColor: t.border }}>
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          ...
                          <p className="text-xs mt-0.5" style={{ color: t.muted }}>{k.created} · {k.lastUsed}</p>
                        </div>
                      </div>
                      <code className="block text-xs px-3 py-2 rounded-xl font-mono"
                  ...
                  <div>
                    <h4 className="font-semibold text-sm mb-3" style={{ color: t.text }}>Recent sign-ins</h4>
                    <div className="rounded-xl border overflow-hidden" style={{ borderColor: t.border }}>
                      {auditEntries.length === 0 && (
                        <p className="px-4 py-6 text-center text-xs" style={{ color: t.muted }}>No sign-ins recorded yet.</p>
                      )}
                      {auditEntries.slice(0, 12).map((e: any, i: number) => (
                        <div key={i} className="flex items-center gap-3 px-4 py-3 border-b last:border-b-0"
                          style={{ borderColor: t.border }}>
                          <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0"
                            style={{ background: "#F0F7F5" }}>
                            <Shield className="w-3.5 h-3.5" style={{ color: "#5E8B7E" }} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-medium" style={{ color: t.text }}>
                              {e.actor} <span style={{ color: t.muted }}>· {e.role}</span>
                            </p>
                            <p className="text-xs truncate" style={{ color: t.muted }}>{e.device}</p>
                          </div>
                          <span className="text-xs shrink-0" style={{ color: t.muted }}>{e.time}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <AnimatePresence>
                  {saved && (
                    <motion.span initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }}
                      ...
                      style={{ color: "#4CAF50" }}>
                      <Check className="w-4 h-4" /> Saved to the server
                    </motion.span>
                  )}
                  {saveError && (
                    <motion.span initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }}
