...
export function FeedbackPage() {
  ...
  const thisMonth = reviews.filter(r => new Date(r.date).getMonth() === new Date().getMonth()).length;
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1fr', gap: 20 }}>
        ...
        <div style={{ background: colors.white, borderRadius: 20, padding: '28px', boxShadow: shadows.card, border: `1px solid ${colors.border}`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 4, margin: '12px 0 8px' }}>
            {[1,2,3,4,5].map(s => <Star key={s} size={20} fill={colors.warning} color={colors.warning} />)}
          </div>
          <div style={{ fontFamily: 'Inter', fontSize: 13, color: colors.textSecondary }}>{total} total review{total === 1 ? '' : 's'}</div>
          <div style={{ fontFamily: 'Inter', fontSize: 12, color: colors.success, marginTop: 8, fontWeight: 600 }}>Top 5% of doctors ✓</div>
        </div>
          ...
          <div key={review.id} style={{ background: colors.white, borderRadius: 20, padding: '24px', boxShadow: shadows.card, border: `1px solid ${colors.border}` }}>
            ...
            {review.replied && (
              <div style={{ marginTop: 16, padding: '14px', borderRadius: 12, background: colors.veryLightSage, borderLeft: `3px solid ${colors.primary}` }}>
                <div style={{ fontFamily: 'Inter', fontSize: 12, fontWeight: 600, color: colors.primary, marginBottom: 4 }}>Dr. Rachel Morgan · Replied</div>
            ...
            {replyingTo === review.id && (
              <div style={{ marginTop: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                  <button onClick={() => setReplyingTo(null)} style={{ padding: '8px 16px', borderRadius: 10, border: 'none', background: colors.primary, color: 'white', fontFamily: 'Inter', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>Post Reply</button>
                  <button onClick={() => setReplyingTo(null)} style={{ padding: '8px 16px', borderRadius: 10, border: `1px solid ${colors.border}`, background: 'transparent', fontFamily: 'Inter', fontSize: 12, color: colors.textSecondary, cursor: 'pointer' }}>Cancel</button>
                </div>
              </div>
            )}
          </div>
